$(document).ready(function() {	
	...
	function progressBarCrowns(percent, $element) {
		var progressBarCrownsWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarCrownsWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarCrowns(20, $('#progressBarCrowns'));
	var selectCrowns = $( "#Crowns-amount-wrapper" );
	var sliderCrowns = $( "<div id='slider-Crowns'></div>" ).insertAfter( selectCrowns ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueCrowns = $( "#slider-Crowns" ).slider( "option", "value" );
				$('#Crowns-amount').html(sliderValueCrowns);
					if(sliderValueCrowns == '10000') {
						progressBarCrowns(20, $('#progressBarCrowns'));
						$('#decrease-Crowns').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueCrowns == '25000') {
						progressBarCrowns(40, $('#progressBarCrowns'));
						$('#decrease-Crowns').removeClass('btn-disabled');
					}
					else if (sliderValueCrowns == '55000') {
						progressBarCrowns(60, $('#progressBarCrowns'));
					}
					else if (sliderValueCrowns == '70000') {
						progressBarCrowns(80, $('#progressBarCrowns'));
						$('#increase-Crowns').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueCrowns == '99999') {
						progressBarCrowns(100, $('#progressBarCrowns'));
						$('#increase-Crowns').addClass('btn-disabled');
						$('.max-amount-points').fadeIn();
					}
				}        
	});				

	$('#increase-Crowns').click(function() {
		var sliderCurrentCrownsValue = $( "#slider-Crowns" ).slider( "option", "value" );
		sliderCrowns.slider( "value", sliderCurrentCrownsValue + 15000 );
	});

	$('#decrease-Crowns').click(function() {
		var sliderCurrentCrownsValue = $( "#slider-Crowns" ).slider( "option", "value" );
		sliderCrowns.slider( "value", sliderCurrentCrownsValue - 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Crowns_msg = $('#slider-Crowns').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Crowns");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Crowns_msg + "</span> Crowns");
					$('.console-Crowns-wrapper').fadeIn(500, function() {
						var $console_Crowns_countto = $('#slider-Crowns').slider("option", "value");					
						$('#console-Crowns-value').countTo({
							from: 0,
							to: $console_Crowns_countto,
							speed: 2500,
							refreshInterval: 10,
							formatter: function (value, options) {
							  return value.toFixed(options.decimals);
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Crowns_msg + "</span> <span class='console-message-success'> Crowns successfully generated</span>");
					$('#console-success-confirmation-Crowns').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
