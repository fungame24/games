$(document).ready(function() {	
	...
	var select = $( "#Money-amount-wrapper" );
	var slider = $( "<div id='slider-Money'></div>" ).insertAfter( select ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Money" ).slider( "option", "value" );				
			$('#Money-amount').html(sliderValue);
			if(sliderValue == '10000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Money').addClass('btn-disabled');
				$('.max-amount-Money').fadeOut();
			}
			else if (sliderValue == '25000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Money').removeClass('btn-disabled');
			}
				...
				$('#increase-Money').removeClass('btn-disabled');
				$('.max-amount-Money').fadeOut();
			}
			else if (sliderValue == '99999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Money').addClass('btn-disabled');
				$('.max-amount-Money').fadeIn();
			}
		}        
	});	
	
	$('#increase-Money').click(function() {
	var sliderCurrentValue = $( "#slider-Money" ).slider( "option", "value" );
	...
	function progressBarMoney(percent, $element) {
		var progressBarMoneyWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarMoneyWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarMoney(20, $('#progressBarMoney'));
	var selectMoney = $( "#Money-amount-wrapper" );
	var sliderMoney = $( "<div id='slider-Money'></div>" ).insertAfter( selectMoney ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueMoney = $( "#slider-Money" ).slider( "option", "value" );
				$('#Money-amount').html(sliderValueMoney);
					if(sliderValueMoney == '10000') {
						progressBarMoney(20, $('#progressBarMoney'));
						$('#decrease-Money').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueMoney == '25000') {
						progressBarMoney(40, $('#progressBarMoney'));
						$('#decrease-Money').removeClass('btn-disabled');
					}
					else if (sliderValueMoney == '55000') {
						progressBarMoney(60, $('#progressBarMoney'));
					}
					else if (sliderValueMoney == '70000') {
						progressBarMoney(80, $('#progressBarMoney'));
						$('#increase-Money').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueMoney == '99999') {
						progressBarMoney(100, $('#progressBarMoney'));
						$('#increase-Money').addClass('btn-disabled');
	...
	$('#increase-Money').click(function() {
		var sliderCurrentMoneyValue = $( "#slider-Money" ).slider( "option", "value" );
		sliderMoney.slider( "value", sliderCurrentMoneyValue + 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Money_msg = $('#slider-Money').slider("option", "value");
				var $console_message_Money_msg = $('#slider-Money').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Money");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Money_msg + "</span> Money");
					$('.console-Money-wrapper').fadeIn(500, function() {
						var $console_Money_countto = $('#slider-Money').slider("option", "value");
						$('#console-Money-value').countTo({
							from: 0,
							to: $console_Money_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Money_msg + "</span> <span class='console-message-success'>Money successfully generated</span>");
					$('#console-success-confirmation-Money').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
