$(document).ready(function() {	
	...
	function progressBarCoins(percent, $element) {
		var progressBarCoinsWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarCoinsWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarCoins(20, $('#progressBarCoins'));
	var selectCoins = $( "#Coins-amount-wrapper" );
	var sliderCoins = $( "<div id='slider-Coins'></div>" ).insertAfter( selectCoins ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueCoins = $( "#slider-Coins" ).slider( "option", "value" );
				$('#Coins-amount').html(sliderValueCoins);
					if(sliderValueCoins == '10000') {
						progressBarCoins(20, $('#progressBarCoins'));
						$('#decrease-Coins').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueCoins == '25000') {
						progressBarCoins(40, $('#progressBarCoins'));
						$('#decrease-Coins').removeClass('btn-disabled');
					}
					else if (sliderValueCoins == '55000') {
						progressBarCoins(60, $('#progressBarCoins'));
					}
					else if (sliderValueCoins == '70000') {
						progressBarCoins(80, $('#progressBarCoins'));
						$('#increase-Coins').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueCoins == '99999') {
						progressBarCoins(100, $('#progressBarCoins'));
						$('#increase-Coins').addClass('btn-disabled');
	...
	$('#increase-Coins').click(function() {
		var sliderCurrentCoinsValue = $( "#slider-Coins" ).slider( "option", "value" );
		sliderCoins.slider( "value", sliderCurrentCoinsValue + 15000 );
	});
