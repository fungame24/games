$(document).ready(function() {	
	...
	function progressBarSouls(percent, $element) {
		var progressBarSoulsWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarSoulsWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarSouls(20, $('#progressBarSouls'));
	var selectSouls = $( "#Souls-amount-wrapper" );
	var sliderSouls = $( "<div id='slider-Souls'></div>" ).insertAfter( selectSouls ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueSouls = $( "#slider-Souls" ).slider( "option", "value" );
				$('#Souls-amount').html(sliderValueSouls);
					if(sliderValueSouls == '10000') {
						progressBarSouls(20, $('#progressBarSouls'));
						$('#decrease-Souls').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueSouls == '25000') {
						progressBarSouls(40, $('#progressBarSouls'));
						$('#decrease-Souls').removeClass('btn-disabled');
					}
					else if (sliderValueSouls == '55000') {
						progressBarSouls(60, $('#progressBarSouls'));
					}
					else if (sliderValueSouls == '70000') {
						progressBarSouls(80, $('#progressBarSouls'));
						$('#increase-Souls').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueSouls == '99999') {
						progressBarSouls(100, $('#progressBarSouls'));
						$('#increase-Souls').addClass('btn-disabled');
						$('.max-amount-points').fadeIn();
					}
				}        
	});				

	$('#increase-Souls').click(function() {
		var sliderCurrentSoulsValue = $( "#slider-Souls" ).slider( "option", "value" );
		sliderSouls.slider( "value", sliderCurrentSoulsValue + 15000 );
	});

	$('#decrease-Souls').click(function() {
		var sliderCurrentSoulsValue = $( "#slider-Souls" ).slider( "option", "value" );
		sliderSouls.slider( "value", sliderCurrentSoulsValue - 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Souls_msg = $('#slider-Souls').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Souls");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Souls_msg + "</span> Souls");
					$('.console-Souls-wrapper').fadeIn(500, function() {
						var $console_Souls_countto = $('#slider-Souls').slider("option", "value");					
						$('#console-Souls-value').countTo({
							from: 0,
							to: $console_Souls_countto,
							speed: 2500,
							refreshInterval: 10,
							formatter: function (value, options) {
							  return value.toFixed(options.decimals);
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Souls_msg + "</span> <span class='console-message-success'> Souls successfully generated</span>");
					$('#console-success-confirmation-Souls').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
