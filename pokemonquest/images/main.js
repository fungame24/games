$(document).ready(function() {	
	...
	var select = $( "#Tickets-amount-wrapper" );
	var slider = $( "<div id='slider-Tickets'></div>" ).insertAfter( select ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Tickets" ).slider( "option", "value" );				
			$('#Tickets-amount').html(sliderValue);
			if(sliderValue == '100000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Tickets').addClass('btn-disabled');
				$('.max-amount-Tickets').fadeOut();
			}
			else if (sliderValue == '250000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Tickets').removeClass('btn-disabled');
			}
				...
				$('#increase-Tickets').removeClass('btn-disabled');
				$('.max-amount-Tickets').fadeOut();
			}
			else if (sliderValue == '999999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Tickets').addClass('btn-disabled');
				$('.max-amount-Tickets').fadeIn();
			}
		}        
	});	
	
	$('#increase-Tickets').click(function() {
	var sliderCurrentValue = $( "#slider-Tickets" ).slider( "option", "value" );
	...
	function progressBarTickets(percent, $element) {
		var progressBarTicketsWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarTicketsWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarTickets(20, $('#progressBarTickets'));
	var selectTickets = $( "#Tickets-amount-wrapper" );
	var sliderTickets = $( "<div id='slider-Tickets'></div>" ).insertAfter( selectTickets ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueTickets = $( "#slider-Tickets" ).slider( "option", "value" );
				$('#Tickets-amount').html(sliderValueTickets);
					if(sliderValueTickets == '100000') {
						progressBarTickets(20, $('#progressBarTickets'));
						$('#decrease-Tickets').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueTickets == '250000') {
						progressBarTickets(40, $('#progressBarTickets'));
						$('#decrease-Tickets').removeClass('btn-disabled');
					}
					else if (sliderValueTickets == '550000') {
						progressBarTickets(60, $('#progressBarTickets'));
					}
					else if (sliderValueTickets == '700000') {
						progressBarTickets(80, $('#progressBarTickets'));
						$('#increase-Tickets').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueTickets == '999999') {
						progressBarTickets(100, $('#progressBarTickets'));
						$('#increase-Tickets').addClass('btn-disabled');
	...
	$('#increase-Tickets').click(function() {
		var sliderCurrentTicketsValue = $( "#slider-Tickets" ).slider( "option", "value" );
		sliderTickets.slider( "value", sliderCurrentTicketsValue + 150000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Tickets_msg = $('#slider-Tickets').slider("option", "value");
				var $console_message_Tickets_msg = $('#slider-Tickets').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Tickets");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Tickets_msg + "</span> Tickets");
					$('.console-Tickets-wrapper').fadeIn(500, function() {
						var $console_Tickets_countto = $('#slider-Tickets').slider("option", "value");
						$('#console-Tickets-value').countTo({
							from: 0,
							to: $console_Tickets_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Tickets_msg + "</span> <span class='console-message-success'>Tickets successfully generated</span>");
					$('#console-success-confirmation-Tickets').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
