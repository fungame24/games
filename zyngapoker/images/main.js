$(document).ready(function() {	
	...
	var select = $( "#Chips-amount-wrapper" );
	var slider = $( "<div id='slider-Chips'></div>" ).insertAfter( select ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Chips" ).slider( "option", "value" );				
			$('#Chips-amount').html(sliderValue);
			if(sliderValue == '100000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Chips').addClass('btn-disabled');
				$('.max-amount-Chips').fadeOut();
			}
			else if (sliderValue == '250000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Chips').removeClass('btn-disabled');
			}
				...
				$('#increase-Chips').removeClass('btn-disabled');
				$('.max-amount-Chips').fadeOut();
			}
			else if (sliderValue == '999999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Chips').addClass('btn-disabled');
				$('.max-amount-Chips').fadeIn();
			}
		}        
	});	
	
	$('#increase-Chips').click(function() {
	var sliderCurrentValue = $( "#slider-Chips" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 150000 );
	});

	$('#decrease-Chips').click(function() {
	var sliderCurrentValue = $( "#slider-Chips" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 150000 );
	});

	function progressBarChips(percent, $element) {
		var progressBarChipsWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarChipsWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarChips(20, $('#progressBarChips'));
	var selectChips = $( "#Chips-amount-wrapper" );
	var sliderChips = $( "<div id='slider-Chips'></div>" ).insertAfter( selectChips ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueChips = $( "#slider-Chips" ).slider( "option", "value" );
				$('#Chips-amount').html(sliderValueChips);
					if(sliderValueChips == '100000') {
						progressBarChips(20, $('#progressBarChips'));
						$('#decrease-Chips').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueChips == '250000') {
						progressBarChips(40, $('#progressBarChips'));
						$('#decrease-Chips').removeClass('btn-disabled');
					}
					else if (sliderValueChips == '550000') {
						progressBarChips(60, $('#progressBarChips'));
					}
					else if (sliderValueChips == '700000') {
						progressBarChips(80, $('#progressBarChips'));
						$('#increase-Chips').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueChips == '999999') {
						progressBarChips(100, $('#progressBarChips'));
						$('#increase-Chips').addClass('btn-disabled');
	...
	$('#increase-Chips').click(function() {
		var sliderCurrentChipsValue = $( "#slider-Chips" ).slider( "option", "value" );
		sliderChips.slider( "value", sliderCurrentChipsValue + 150000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Chips_msg = $('#slider-Chips').slider("option", "value");
				var $console_message_Chips_msg = $('#slider-Chips').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Chips");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Chips_msg + "</span> Chips");
					$('.console-Chips-wrapper').fadeIn(500, function() {
						var $console_Chips_countto = $('#slider-Chips').slider("option", "value");
						$('#console-Chips-value').countTo({
							from: 0,
							to: $console_Chips_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Chips_msg + "</span> <span class='console-message-success'>Chips successfully generated</span>");
					$('#console-success-confirmation-Chips').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
