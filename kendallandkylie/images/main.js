$(document).ready(function() {	
	...
	var select = $( "#Cash-and-Gems-amount-wrapper" );
	var slider = $( "<div id='slider-Cash-and-Gems'></div>" ).insertAfter( select ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Cash-and-Gems" ).slider( "option", "value" );				
			$('#Cash-and-Gems-amount').html(sliderValue);
			if(sliderValue == '100000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Cash-and-Gems').addClass('btn-disabled');
				$('.max-amount-Cash-and-Gems').fadeOut();
			}
			else if (sliderValue == '250000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Cash-and-Gems').removeClass('btn-disabled');
			}
				...
				$('#increase-Cash-and-Gems').removeClass('btn-disabled');
				$('.max-amount-Cash-and-Gems').fadeOut();
			}
			else if (sliderValue == '999999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Cash-and-Gems').addClass('btn-disabled');
				$('.max-amount-Cash-and-Gems').fadeIn();
			}
		}        
	});	
	
	$('#increase-Cash-and-Gems').click(function() {
	var sliderCurrentValue = $( "#slider-Cash-and-Gems" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 150000 );
	});

	$('#decrease-Cash-and-Gems').click(function() {
	var sliderCurrentValue = $( "#slider-Cash-and-Gems" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 150000 );
	});

	function progressBarCash-and-Gems(percent, $element) {
		var progressBarCash-and-GemsWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarCash-and-GemsWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarCash-and-Gems(20, $('#progressBarCash-and-Gems'));
	var selectCash-and-Gems = $( "#Cash-and-Gems-amount-wrapper" );
	var sliderCash-and-Gems = $( "<div id='slider-Cash-and-Gems'></div>" ).insertAfter( selectCash-and-Gems ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueCash-and-Gems = $( "#slider-Cash-and-Gems" ).slider( "option", "value" );
				$('#Cash-and-Gems-amount').html(sliderValueCash-and-Gems);
					if(sliderValueCash-and-Gems == '100000') {
						progressBarCash-and-Gems(20, $('#progressBarCash-and-Gems'));
						$('#decrease-Cash-and-Gems').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueCash-and-Gems == '250000') {
						progressBarCash-and-Gems(40, $('#progressBarCash-and-Gems'));
						$('#decrease-Cash-and-Gems').removeClass('btn-disabled');
					}
					else if (sliderValueCash-and-Gems == '550000') {
						progressBarCash-and-Gems(60, $('#progressBarCash-and-Gems'));
					}
					else if (sliderValueCash-and-Gems == '700000') {
						progressBarCash-and-Gems(80, $('#progressBarCash-and-Gems'));
						$('#increase-Cash-and-Gems').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueCash-and-Gems == '999999') {
						progressBarCash-and-Gems(100, $('#progressBarCash-and-Gems'));
						$('#increase-Cash-and-Gems').addClass('btn-disabled');
	...
	$('#increase-Cash-and-Gems').click(function() {
		var sliderCurrentCash-and-GemsValue = $( "#slider-Cash-and-Gems" ).slider( "option", "value" );
		sliderCash-and-Gems.slider( "value", sliderCurrentCash-and-GemsValue + 150000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Cash-and-Gems_msg = $('#slider-Cash-and-Gems').slider("option", "value");
				var $console_message_Cash-and-Gems_msg = $('#slider-Cash-and-Gems').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Cash-and-Gems");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Cash-and-Gems_msg + "</span> Cash-and-Gems");
					$('.console-Cash-and-Gems-wrapper').fadeIn(500, function() {
						var $console_Cash-and-Gems_countto = $('#slider-Cash-and-Gems').slider("option", "value");
						$('#console-Cash-and-Gems-value').countTo({
							from: 0,
							to: $console_Cash-and-Gems_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Cash-and-Gems_msg + "</span> <span class='console-message-success'>Cash-and-Gems successfully generated</span>");
					$('#console-success-confirmation-Cash-and-Gems').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
