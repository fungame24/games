$(document).ready(function() {	
	...
	var select = $( "#Caps-amount-wrapper" );
	var slider = $( "<div id='slider-Caps'></div>" ).insertAfter( select ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Caps" ).slider( "option", "value" );				
			$('#Caps-amount').html(sliderValue);
			if(sliderValue == '10000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Caps').addClass('btn-disabled');
				$('.max-amount-Caps').fadeOut();
			}
			else if (sliderValue == '25000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Caps').removeClass('btn-disabled');
			}
				...
				$('#increase-Caps').removeClass('btn-disabled');
				$('.max-amount-Caps').fadeOut();
			}
			else if (sliderValue == '99999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Caps').addClass('btn-disabled');
				$('.max-amount-Caps').fadeIn();
			}
		}        
	});	
	
	$('#increase-Caps').click(function() {
	var sliderCurrentValue = $( "#slider-Caps" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 15000 );
	});

	$('#decrease-Caps').click(function() {
	var sliderCurrentValue = $( "#slider-Caps" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 15000 );
	});

	function progressBarQuantum(percent, $element) {
		var progressBarQuantumWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarQuantumWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarQuantum(20, $('#progressBarQuantum'));
	var selectQuantum = $( "#Quantum-amount-wrapper" );
	var sliderQuantum = $( "<div id='slider-Quantum'></div>" ).insertAfter( selectQuantum ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueQuantum = $( "#slider-Quantum" ).slider( "option", "value" );
				$('#Quantum-amount').html(sliderValueQuantum);
					if(sliderValueQuantum == '10000') {
						progressBarQuantum(20, $('#progressBarQuantum'));
						$('#decrease-Quantum').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueQuantum == '25000') {
						progressBarQuantum(40, $('#progressBarQuantum'));
						$('#decrease-Quantum').removeClass('btn-disabled');
					}
					else if (sliderValueQuantum == '55000') {
						progressBarQuantum(60, $('#progressBarQuantum'));
					}
					else if (sliderValueQuantum == '70000') {
						progressBarQuantum(80, $('#progressBarQuantum'));
						$('#increase-Quantum').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueQuantum == '99999') {
						progressBarQuantum(100, $('#progressBarQuantum'));
						$('#increase-Quantum').addClass('btn-disabled');
						$('.max-amount-points').fadeIn();
					}
				}        
	});				

	$('#increase-Quantum').click(function() {
		var sliderCurrentQuantumValue = $( "#slider-Quantum" ).slider( "option", "value" );
		sliderQuantum.slider( "value", sliderCurrentQuantumValue + 15000 );
	});

	$('#decrease-Quantum').click(function() {
		var sliderCurrentQuantumValue = $( "#slider-Quantum" ).slider( "option", "value" );
		sliderQuantum.slider( "value", sliderCurrentQuantumValue - 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Caps_msg = $('#slider-Caps').slider("option", "value");
				var $console_message_Quantum_msg = $('#slider-Quantum').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Caps");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Caps_msg + "</span> Caps");
					$('.console-Caps-wrapper').fadeIn(500, function() {
						var $console_Caps_countto = $('#slider-Caps').slider("option", "value");
						$('#console-Caps-value').countTo({
							from: 0,
							to: $console_Caps_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Caps_msg + "</span> <span class='console-message-success'>Caps successfully generated</span>");
					$('#console-success-confirmation-Caps').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Quantum");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Quantum_msg + "</span> Quantum");
					$('.console-Quantum-wrapper').fadeIn(500, function() {
						var $console_Quantum_countto = $('#slider-Quantum').slider("option", "value");					
						$('#console-Quantum-value').countTo({
							from: 0,
							to: $console_Quantum_countto,
							speed: 2500,
							refreshInterval: 10,
							formatter: function (value, options) {
							  return value.toFixed(options.decimals);
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Quantum_msg + "</span> <span class='console-message-success'> Quantum successfully generated</span>");
					$('#console-success-confirmation-Quantum').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
