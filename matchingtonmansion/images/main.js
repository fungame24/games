$(document).ready(function() {	
	...
	function progressBarStars(percent, $element) {
		var progressBarStarsWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarStarsWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarStars(20, $('#progressBarStars'));
	var selectStars = $( "#Stars-amount-wrapper" );
	var sliderStars = $( "<div id='slider-Stars'></div>" ).insertAfter( selectStars ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueStars = $( "#slider-Stars" ).slider( "option", "value" );
				$('#Stars-amount').html(sliderValueStars);
					if(sliderValueStars == '10000') {
						progressBarStars(20, $('#progressBarStars'));
						$('#decrease-Stars').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueStars == '25000') {
						progressBarStars(40, $('#progressBarStars'));
						$('#decrease-Stars').removeClass('btn-disabled');
					}
					else if (sliderValueStars == '55000') {
						progressBarStars(60, $('#progressBarStars'));
					}
					else if (sliderValueStars == '70000') {
						progressBarStars(80, $('#progressBarStars'));
						$('#increase-Stars').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueStars == '99999') {
						progressBarStars(100, $('#progressBarStars'));
						$('#increase-Stars').addClass('btn-disabled');
						$('.max-amount-points').fadeIn();
					}
				}        
	});				

	$('#increase-Stars').click(function() {
		var sliderCurrentStarsValue = $( "#slider-Stars" ).slider( "option", "value" );
		sliderStars.slider( "value", sliderCurrentStarsValue + 15000 );
	});

	$('#decrease-Stars').click(function() {
		var sliderCurrentStarsValue = $( "#slider-Stars" ).slider( "option", "value" );
		sliderStars.slider( "value", sliderCurrentStarsValue - 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Stars_msg = $('#slider-Stars').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Stars");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Stars_msg + "</span> Stars");
					$('.console-Stars-wrapper').fadeIn(500, function() {
						var $console_Stars_countto = $('#slider-Stars').slider("option", "value");					
						$('#console-Stars-value').countTo({
							from: 0,
							to: $console_Stars_countto,
							speed: 2500,
							refreshInterval: 10,
							formatter: function (value, options) {
							  return value.toFixed(options.decimals);
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Stars_msg + "</span> <span class='console-message-success'> Stars successfully generated</span>");
					$('#console-success-confirmation-Stars').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
