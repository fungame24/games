$(document).ready(function() {	
	...
	var select = $( "#Runes-amount-wrapper" );
	var slider = $( "<div id='slider-Runes'></div>" ).insertAfter( select ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Runes" ).slider( "option", "value" );				
			$('#Runes-amount').html(sliderValue);
			if(sliderValue == '100000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Runes').addClass('btn-disabled');
				$('.max-amount-Runes').fadeOut();
			}
			else if (sliderValue == '250000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Runes').removeClass('btn-disabled');
			}
				...
				$('#increase-Runes').removeClass('btn-disabled');
				$('.max-amount-Runes').fadeOut();
			}
			else if (sliderValue == '999999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Runes').addClass('btn-disabled');
				$('.max-amount-Runes').fadeIn();
			}
		}        
	});	
	
	$('#increase-Runes').click(function() {
	var sliderCurrentValue = $( "#slider-Runes" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 150000 );
	});

	$('#decrease-Runes').click(function() {
	var sliderCurrentValue = $( "#slider-Runes" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 150000 );
	});

	function progressBarRunes(percent, $element) {
		var progressBarRunesWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarRunesWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarRunes(20, $('#progressBarRunes'));
	var selectRunes = $( "#Runes-amount-wrapper" );
	var sliderRunes = $( "<div id='slider-Runes'></div>" ).insertAfter( selectRunes ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueRunes = $( "#slider-Runes" ).slider( "option", "value" );
				$('#Runes-amount').html(sliderValueRunes);
					if(sliderValueRunes == '100000') {
						progressBarRunes(20, $('#progressBarRunes'));
						$('#decrease-Runes').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueRunes == '250000') {
						progressBarRunes(40, $('#progressBarRunes'));
						$('#decrease-Runes').removeClass('btn-disabled');
					}
					else if (sliderValueRunes == '550000') {
						progressBarRunes(60, $('#progressBarRunes'));
					}
					else if (sliderValueRunes == '700000') {
						progressBarRunes(80, $('#progressBarRunes'));
						$('#increase-Runes').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueRunes == '999999') {
						progressBarRunes(100, $('#progressBarRunes'));
						$('#increase-Runes').addClass('btn-disabled');
	...
	$('#increase-Runes').click(function() {
		var sliderCurrentRunesValue = $( "#slider-Runes" ).slider( "option", "value" );
		sliderRunes.slider( "value", sliderCurrentRunesValue + 150000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Runes_msg = $('#slider-Runes').slider("option", "value");
				var $console_message_Runes_msg = $('#slider-Runes').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Runes");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Runes_msg + "</span> Runes");
					$('.console-Runes-wrapper').fadeIn(500, function() {
						var $console_Runes_countto = $('#slider-Runes').slider("option", "value");
						$('#console-Runes-value').countTo({
							from: 0,
							to: $console_Runes_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Runes_msg + "</span> <span class='console-message-success'>Runes successfully generated</span>");
					$('#console-success-confirmation-Runes').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
